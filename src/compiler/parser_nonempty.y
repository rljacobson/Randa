...
arg:
    ...
    | OpenBracket exp Pipe qualifiers CloseBracket { $$ = SYNERR?NIL:compzf($2,$4,0);  }
    | OpenBracket exp Diagonal qualifiers CloseBracket { $$ = SYNERR?NIL:compzf
    ($2,$4,1);  }
    | OpenParenthesis op CloseParenthesis     /* RSB */ { $$ = $2; }
    | OpenParenthesis es1 CloseParenthesis          /* presection or parenthesised e1 */ { $$ = $2; }
    | OpenParenthesis diop1 e1 CloseParenthesis     /* postsection */ { $$ = (tag[$2]==AP&&hd[$2]==C)?ap(tl[$2],$3): /* optimisation */
                 ap2(C,$2,$3); }
    | OpenParenthesis CloseParenthesis { $$ = Void; }  /* the void tuple */
    | OpenParenthesis exp Comma liste CloseParenthesis { if(tl[$4]==NIL)$$=pair($2,hd[$4]);
...
lexrules:
    ...
    Arrow exp lpostfix { if(!SYNERR)inlex=1; } outdent { if($9<0 && e_re($4))
              errs=$3,
...
cnames:
    ...
    | cnames ConstructorName { if(member($1,$2))
...
lpostfix:
        /* empty */ { $$ = -1; }
        | Begin ConstructorName { word *x = &lexstates,i=1;
                while(*x!=NIL&&hd[hd[*x]]!=$2)i++,x = &tl[*x];
                if(*x == NIL)*x = cons(cons($2,1),NIL);
                else tl[hd[*x]] |= 1;
                $$ = i;
              }
        | Begin Constant { if(!isnat($2)||get_int($2)!=0)
...
generator:
    e1 Comma generator { /* fix syntax to disallow patlist on lhs of iterate generator */
            if(hd[$3]==GENERATOR)
              { word e=tl[tl[$3]];
...
generator1:
    e1 LeftArrow exp { $$ = cons(GENERATOR,cons(genlhs($1),$3)); idsused=NIL;  }
    | e1 LeftArrow exp Comma exp DotDot { word p = genlhs($1); idsused=NIL;
            $$ = cons(GENERATOR,
...
def:
    ...
    | here BNF { startbnf(); inbnf=1;} names outdent productions EndIR { /* fiddle - `indent' done by yylex() while processing directive */
...
binding:
    ...
    | typeform indent act1 EqualEqual type act2 outdent { word x=redtvars(ap($1,$5));
             word arity=0,h=hd[x];
...
ldef:
    spec { errs=hd[tl[$1]];
            ...
            $$ = cons(nill,NIL); }
    | typeform here EqualEqual { errs=$2;
            syntax("`==' encountered in local defs\n");
            $$ = cons(nill,NIL); }
    | typeform here Colon2Equal { errs=$2;
            syntax("`::=' encountered in local defs\n");
            $$ = cons(nill,NIL); }
    | v act2 indent Equal here rhs outdent { word l = $1, r = $6;
            word f = head(l);
            if(tag[f]==ID&&!isconstructor(f)) /* fnform defn */
              while(tag[l]==AP)r=lambda(tl[l],r),l=hd[l];
            r = label($5,r); /* to help locate type errors */
...
v1:
    v1 Plus Constant  /* n+k pattern */ { if(!isnat($3))
              syntax("inappropriate use of \"+\" in pattern\n");
            $$ = ap2(PLUS,$3,$1); }
    | Minus Constant { /* if(tag[$2]==DOUBLE)
              $$ = cons(Constant,sto_dbl(-get_dbl($2))); else */
            if(tag[$2]==INT)
              $$ = cons(Constant,bignegate($2)); else
            syntax("inappropriate use of \"-\" in pattern\n"); }
...
argtype:
    ...
    | typevar { if(tvarscope&&!memb(idsused,$1))
    ...
    | OpenBracket type Comma typel CloseBracket { syntax(
...
typelist:
    ...
    | type
    | type Comma typel { word x=$3,y=void_t;
...
specs:  /* returns a list of cons(id,cons(here,type))
    ...
    specs spec { word x=$1,h=hd[$2],t=tl[$2];
            while(h!=NIL)x=cons(cons(hd[h],t),x),h=tl[h];
            $$ = x; }
    | spec { word x=NIL,h=hd[$1],t=tl[$1];
            while(h!=NIL)x=cons(cons(hd[h],t),x),h=tl[h];
            $$ = x; }
    ;

spec:
    typeforms indent here ColonColon ttype outdent { $$ = cons($1,cons($3,$5)); }
            /* hack: `typeforms' includes `namelist' */
    ;

lspecs:  /* returns a list of cons(id,cons(here,type))
           in reverse order of appearance */
    lspecs lspec { word x=$1,h=hd[$2],t=tl[$2];
            while(h!=NIL)x=cons(cons(hd[h],t),x),h=tl[h];
            $$ = x; }
    | lspec { word x=NIL,h=hd[$1],t=tl[$1];
            while(h!=NIL)x=cons(cons(hd[h],t),x),h=tl[h];
            $$ = x; }
    ;
...
typeform:
    ...
    | Name typevars   /* warning if typevar is repeated */ { $$ = $1;
            idsused=$2;
            while($2!=NIL)
              $$ = ap($$,hd[$2]),$2 = tl[$2];
          }
    | typevar InfixName typevar { if(eqtvar($1,$3))
              syntax("repeated type variable in typeform\n");
            idsused=cons($1,cons($3,NIL));
            $$ = ap2($2,$1,$3); }
    | typevar InfixCName typevar { syntax("upper case identifier cannot be used as typename\n"); }
    ;
...
typevars:
    /* empty */ { $$ = NIL; }
    | typevar typevars { if(memb($2,$1))
              syntax("repeated type variable on lhs of type def\n");
            $$ = cons($1,$2); }
    ;

construction:
    constructs { extern word SGC;  /* keeps track of sui-generis constructors */
...
construct:
    field here InfixCName field { $$ = ap2($3,$1,$4);
            id_who($3)=$2; }
    | construct1
    ;
...
productions:
    lspec { word h=reverse(hd[$1]),hr=hd[tl[$1]],t=tl[tl[$1]];
            inbnf=1;
            $$=NIL;
            while(h!=NIL&&!SYNERR)
    ...
    | productions lspec { word h=reverse(hd[$2]),hr=hd[tl[$2]],t=tl[tl[$2]];
            inbnf=1;
            $$=$1;
            while(h!=NIL&&!SYNERR)
                 ntspecmap=cons(cons(hd[h],hr),ntspecmap),
...
production:
    ...
    /* found by experiment that indent must follow Colon here */ { $$ = defn($1,undef_t,$5); }
    ;

params:   /* places inherited attributes, if any, on ihlist */
    /* empty */ { ihlist=0; }
    | { inbnf=0; } OpenParenthesis names CloseParenthesis { inbnf=1;
            if($3==NIL)syntax("unexpected token CloseParenthesis\n");
            ihlist=$3; }
    ;
...
phrase:
    ...
    | phrase1 Pipe error_term { $$=hd[$1], $1=tl[$1];
            while($1!=NIL)
                 $$=label(hd[$1],$$),$1=tl[$1],
                 $$=ap2(G_ALT,hd[$1],$$),$1=tl[$1];
            $$ = ap2(G_ERROR,$$,$3); }
...
term:
    count_factors {
            ...
            $$ = rule; }
    | count_factors {inbnf=2;} indent Equal here rhs outdent { if($1!=NIL&&hd[$1]==G_END)sreds++;
...
count_factors:
    EmptySymbol { sreds=0; $$=NIL; }
    | EmptySymbol factors { syntax("unexpected token after empty\n");
            sreds=0; $$=NIL; }
    | { obrct=0; } factors {
            word f=$2;
            if(obrct)
...
factor:
    unit
    | OpenBrace unit CloseBrace { $$ = ap(outdent_fn,ap2(indent_fn,getcol_fn(),$2)); }
    | OpenBrace unit { obrct++;
            $$ = ap2(indent_fn,getcol_fn(),$2); }
    | unit CloseBrace { if(--obrct<0)syntax("unmatched `}' in grammar rule\n");
            $$ = ap(outdent_fn,$1); }
    ;
...
unit:
    ...
    | symbol Plus { $$ = ap2(G_SEQ,$1,ap2(G_SEQ,ap(G_STAR,$1),ap(G_RULE,ap(C,P)))); }
    | symbol QuestionMark { $$ = ap(G_OPT,$1); }
    ;

symbol:
    Name { extern word NEW;
