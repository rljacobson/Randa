...
impl Identifier {
    ...
    /// of the identifier.
    pub fn compile(&self, heap: &mut Heap) -> RawValue {
        let who = self.definition.compile(heap);
        let string_ref = heap.string(self.name.clone());
        let mut id_head = heap.strcons(string_ref, who.into());
        id_head = heap.cons(id_head, self.datatype.into());
        let value = if let Some(value) = &self.value {
            value.compile(heap)
        } else {
            Combinator::Undef as RawValue
        };

        let id_ref = heap.put_cell(HeapCell::new(Tag::Id, id_head, value));
    ...
    pub fn get(reference: Value, heap: &Heap) -> Result<Identifier, ()> {
        ...
        let value: Option<IdentifierValue> = IdentifierValue::get(id_cell.tail.into(), heap)?;
        ...
        let id_definition: IdentifierDefinition =
            IdentifierDefinition::get(strcons_cell.tail.into(), heap)?;
...
impl IdentifierDefinition {
    ...
    pub fn compile(&self, heap: &mut Heap) -> RawValue {
        match self {
            ...
            IdentifierDefinition::HereInfo(here_info) => {
                ...
                let here_info = heap.file_info(script, here_info.line_number as RawValue);
                here_info.into()
            }
            ...
            IdentifierDefinition::Alias { here_info, source } => {
                ...
                let h_aka = heap.data_pair(h_source, 0);
                let h_script = heap.string(&here_info.script_file);
                let h_here_info = heap.file_info(h_script, here_info.line_number as RawValue);

                heap.cons(h_aka, h_here_info)
            }
        }
    }
    ...
    pub fn get(reference: Value, heap: &Heap) -> Result<IdentifierDefinition, ()> {
        ...
        if who_cell.tag == Tag::Cons {
            ...
            let (script_file, line_number): (HeapString, LineNumber) =
                IdentifierDefinition::get_here_info(who_cell.tail.into(), heap)?;
            ...
            let (script_file, line_number): (String, LineNumber) =
                IdentifierDefinition::get_here_info(reference, heap)?;

            Ok(IdentifierDefinition::HereInfo(HereInfo {
                script_file,
                line_number,
            }))
        } else {
    ...
    fn get_here_info(reference: Value, heap: &Heap) -> Result<(String, LineNumber), ()> {
        ...
        let line_number: LineNumber = LineNumber::from(here_info_cell.tail);
    ...
    /*
    fn get_here_info_unchecked<'t>(&'t self, reference: RawValue) -> (&'t str, LineNumber){
      let here_info_cell: HeapCell   = heap.data[reference - ATOM_LIMIT];
      let script_file   : &str       = heap.strings[here_info_cell.head as usize].as_str();
    ...
    */
    ...
    /*
    fn get_who_info_unchecked<'t>(reference: RawValue, heap: &Heap) -> IdentifierDefinition<'t> {
      if reference == Combinator::Nil as RawValue {
        // It's not a reference but a Nil combinator, which means the variable is undefined.
      ...
      // Check if the alias info is cons'ed on to the here info cell.
      if who_cell.tag == Tag::Cons {
        let aka_cell = heap.data[(who_cell.head - ATOM_LIMIT) as usize];
        let (script_file, line): (&str, LineNumber) = heap.get_here_info_unchecked(who_cell.tail);
        let source: &str = heap.strings[aka_cell.head as usize].as_str();
    ...
    pub fn compile(&self, heap: &mut Heap) -> RawValue {
        ...
        match self {
            ...
            IdentifierValue::Typed {
                ...
                let value_head = heap.cons(*arity, RawValue::from(*show_function));
                let value_tail = value_type.compile(heap);
                heap.cons(value_head, value_tail.into())
            }

            IdentifierValue::Arbitrary(v) => (*v).into(),
        }
    }
    ...
    pub fn get(reference: Value, heap: &Heap) -> Result<Option<IdentifierValue>, ()> {
        if reference == Combinator::Undef.into() {
            ...
            if let Value::Reference(idx) = reference {
                let outer_cons_cell = heap[idx];
                if outer_cons_cell.tag == Tag::Cons {
                    let inner_cons_cell: HeapCell =
                        heap.expect(Tag::Cons, outer_cons_cell.head.into())?;
                    let show_function = inner_cons_cell.tail.into();
                    let arity: RawValue = inner_cons_cell.head;
                    let value_type = IdentifierValueType::get(outer_cons_cell.tail.into(), heap)?;

                    Ok(Some(IdentifierValue::Typed {
                        arity,
                        show_function,
                        value_type,
                    }))
...
impl IdentifierValueType {
    pub fn compile(&self, heap: &mut Heap) -> RawValue {
        ...
        match self {
            ...
            IdentifierValueType::Free => heap.cons(Value::Data(4).into(), Combinator::Nil.into()),
        } // end match on self
    }

    fn get(reference: Value, heap: &Heap) -> Result<IdentifierValueType, ()> {
        let type_cell: HeapCell = heap.expect(Tag::Cons, reference)?;
        let class_ = match type_cell.head {
            0 => IdentifierValueType::Algebraic {
                constructors: type_cell.tail.into(),
            },

            1 => IdentifierValueType::Synonym {
                source_type: Type::from_isize(type_cell.tail).ok_or(())?,
            },

            2 => IdentifierValueType::Abstract {
                basis: type_cell.tail.into(),
            },

            3 => IdentifierValueType::PlaceHolder,

            4 => IdentifierValueType::Free,
